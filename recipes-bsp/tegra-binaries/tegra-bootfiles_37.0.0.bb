...
install_other_boot_firmware_files() {
    case "${SOC_FAMILY}" in
	tegra234|tegra254)
	    install -m 0644 ${S}/bootloader/tegra*.dts* ${D}${datadir}/tegraflash/
	    install -m 0644 ${S}/bootloader/fuse_*.xml ${D}${datadir}/tegraflash/
	    install -m 0644 ${S}/bootloader/${NVIDIA_BOARD}/tegra*-bpmp-*.dtb ${D}${datadir}/tegraflash/
	    install -m 0644 ${S}/bootloader/${NVIDIA_BOARD}/BCT/tegra* ${D}${datadir}/tegraflash/
	    install -m 0644 ${S}/bootloader/bpmp_t*.bin ${D}${datadir}/tegraflash/
	    ;;
	*)
	    bberror "Unrecognized SOC_FAMILY: ${SOC_FAMILY}"
	    ;;
